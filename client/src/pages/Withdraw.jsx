...
export default function Withdraw() {
  ...
  const handleSearch = async (e) => {
    ...
    if (matches.length === 0) setMessage("No customers found.");
    if (matches.length === 1) setSelectedCustomer(matches[0]);
  };
  ...
  const handleWithdraw = async (e) => {
    ...
    if (Number(amount) > selectedCustomer.balance) {
      setMessage("Insufficient funds.");
      return;
    }
    const res = await fetch(`http://localhost:3001/api/customers/${selectedCustomer._id}/withdraw`, {
      method: "PATCH",
    ...
    <div className="container mt-5">
      ...
      {selectedCustomer && (
        <div className="card p-3 mb-3">
          ...
          <form onSubmit={handleWithdraw} className="row g-2 align-items-center">
            <div className="col-auto">
              <input
                type="number"
                className="form-control"
